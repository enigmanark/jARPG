...
public class CourierToAndReturn extends Quest {
	...
	private boolean itemsDelivered = false;
	
	public CourierToAndReturn(int id) {
		super(id, QuestsType.CourierToAndReturn);
	}
	...
	public void post(Evermore game, Event e) {
		if(e instanceof ActivatedObject) {
			...
			else if(o.object.getName().equals(to) && !itemsDelivered) {
				if(hasItems(game)) {
					deliverItems(game);
				}
				else this.postToLogic(game, new Message("", "You don't have the items to be delivered."));
			}
	...
	private void giveItems(Evermore game) {
		game.dataSys.player.itemMngr.addToBackPack(item, itemAmount);
		if(item2 != null) {
			game.dataSys.player.itemMngr.addToBackPack(item2, item2Amount);
		}
		if(item3 != null) {
			game.dataSys.player.itemMngr.addToBackPack(item3, item3Amount);
		}
		itemsGiven = true;
	}
	...
	private void reward(Evermore game) {
		...
		this.postToLogic(game, new Message("", rewardMessage));
	}
}
